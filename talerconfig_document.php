...
$langs->loadLangs(array("talerbarr@talerbarr", "companies", "other", "mails"));
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->talerbarr->multidir_output[$object->entity ? $object->entity : $conf->entity]."/talerconfig/".get_exdir(0, 0, 0, 1, $object);
}
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd  = $user->hasRight('talerbarr', 'talerconfig', 'write'); // Used by the include of actions_addupdatedelete.inc.php and actions_linkedfiles.inc.php
} else {
	$permissiontoread = 1;
	$permissiontoadd  = 1;
}
...
// Header
// ------
$title = $langs->trans("TalerConfig")." - ".$langs->trans("Files");
//$title = $object->ref." - ".$langs->trans("Files");
$help_url = '';
//Example $help_url='EN:Module_Third_Parties|FR:Module_Tiers|ES:Empresas';
llxHeader('', $title, $help_url, '', 0, 0, '', '', '', 'mod-talerbarr page-card_document');

// Show tabs
$head = talerconfigPrepareHead($object);

print dol_get_fiche_head($head, 'document', $langs->trans("TalerConfig"), -1, $object->picto);
...
$modulepart = 'talerbarr';
$param = '&id='.$object->id;
//$relativepathwithnofile='talerconfig/' . dol_sanitizeFileName($object->id).'/';
$relativepathwithnofile = 'talerconfig/'.dol_sanitizeFileName($object->ref).'/';
