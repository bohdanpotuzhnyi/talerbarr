...
llxHeader("", $langs->trans("TalerBarrArea"), '', '', 0, 0, array(), $morecss, '', 'mod-talerbarr page-index');
...
// --- Orders snapshot ---
$ordersSql = "SELECT rowid, taler_instance, taler_order_id, order_amount_str, taler_state, taler_refund_pending, taler_refunded_total, order_summary, fk_soc, fk_commande, fk_facture, fk_paiement, taler_status_url, taler_pay_deadline, tms";
...
print '<div class="taler-home-grid taler-home-chart taler-home-grid--chart">';
...
print '<select id="taler-orders-metric" class="taler-select-inline"><option value="count" selected>'.dol_escape_htmltag($langs->transnoentities('NumberOfOrders', 'Number of orders')).'</option><option value="amount">'.dol_escape_htmltag($langs->transnoentities('OrderAmounts', 'Order amounts')).'</option></select>';
...
print '<div id="taler-orders-chart-tooltip" class="taler-chart-tooltip taler-hidden"></div>';
print '<div id="taler-orders-chart-empty" class="taler-chart-empty taler-hidden">'.$langs->trans('NoData').'</div>';
print '</div>';
print '</div>';

// Row 2: orders + products glance
print '<div class="taler-home-grid taler-home-middle taler-home-grid--middle">';
...
foreach ($orders as $row) {
	...
	if (!empty($row->taler_refund_pending)) {
		$ordersRefundPending++;
	}
...
if (empty($orders)) {
	...
	print '<div class="taler-stat"><div class="label">Refunded</div><div class="value">'.(int) $ordersRefunded.'</div></div>';
	print '<div class="taler-stat"><div class="label">Refund pending</div><div class="value">'.(int) $ordersRefundPending.'</div></div>';
...
print '<div class="taler-home-grid taler-home-bottom taler-home-grid--bottom">';
...
if (!empty($singleton) && (empty($singleton->verification_ok) || $singleton->verification_ok)) {
	...
	if (!empty($singleton->fk_default_customer)) {
		...
		$syncTimingHtml = '<span class="opacitymedium">'.dol_escape_htmltag($langs->trans('SyncTimingNotApplicable')).'</span>';
	if (isset($singleton->syncdirection) && (int) $singleton->syncdirection === 1) {
		$syncTimingHtml = !empty($singleton->sync_on_paid)
			? dol_escape_htmltag($langs->trans('SyncAtOrderPaid'))
			: dol_escape_htmltag($langs->trans('SyncAtOrderCreated'));
	}
	...
	$configRows = array(
		...
		$langs->trans('SyncDirection') => $directionDisplay,
			$langs->trans('SyncTiming') => $syncTimingHtml,
...
if ($user->admin) {
	print '<span class="right-actions"><form method="POST" action="'.$_SERVER['PHP_SELF'].'" class="taler-inline-form"><input type="hidden" name="token" value="'.newToken().'"><input type="hidden" name="action" value="runsync"><input type="submit" class="button" value="'.$langs->trans("RunSyncNow").'"></form></span>';
}
print '</div>';
if (!$status) {
...
(function() {
	...
	var phaseMap = {
		...
		total: "'.$langs->transnoentities('SyncMetricTotal').'",
			...
			rawData: "'.$langs->transnoentities('RawData').'",
	...
	var ACTIVE_DELAY = 1000;
		...
		if (status.processed !== undefined) {
			...
			try {
				...
				html.push("<details class=\"taler-collapsible taler-sync-debug-collapsible\">");
				html.push("<summary class=\"taler-collapsible-summary\">" + labels.rawData + " (JSON)</summary>");
				html.push("<div class=\"taler-collapsible-body\">");
				html.push("<div class=\"taler-sync-debug\"><pre>" + escapeHtml(pretty) + "</pre></div>");
				html.push("</div>");
				html.push("</details>");
			} catch(e) {
				/* ignore */
