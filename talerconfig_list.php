...
/**
 *   	\file       talerconfig_list.php
 *		\ingroup    talerbarr
 *		\brief      List page for talerconfig
 *
 * @package    Application
 */
...
include_once __DIR__.'/class/talerconfig.class.php';
// for other modules
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('talerbarr', 'talerconfig', 'write');
	$permissiontodelete = $user->hasRight('talerbarr', 'talerconfig', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'TalerConfig';
	$objectlabel = 'TalerConfig';
...
$title = $langs->trans("TalerConfigs");
//$help_url = "EN:Module_TalerConfig|FR:Module_TalerConfig_FR|ES:Módulo_TalerConfig";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/talerbarr/talerconfig_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-talerbarr page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for a horizontal scroll in the table instead of page
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/talerbarr/talerconfig_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, $object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendTalerConfigRef";
$modelmail = "talerconfig";
$objecttmp = new TalerConfig($db);
