...
if ($enablepermissioncheck) {
	...
	$permissiontoadd    = $user->hasRight('talerbarr', 'talerorderlink', 'write');
	$permissiontodelete = $user->hasRight('talerbarr', 'talerorderlink', 'delete');
} else {
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'TalerOrderLink';
	$objectlabel = 'TalerOrderLink';
...
$title   = $langs->trans("TalerOrderLinks");
$help_url = '';
$morejs  = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/talerbarr/talerorderlink_card.php', 1).'?id='.(int) $id);
	exit;
}
...
//$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/talerbarr/talerorderlink_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, $object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Massaction prezone
$topicmail = "SendTalerOrderLinkRef";
$modelmail = "talerorderlink";
$objecttmp = new TalerOrderLink($db);
	...
	if ($mode == 'kanban' || $mode == 'kanbangroupby') {
		...
		foreach ($object->fields as $key => $val) {
			...
			if (!empty($arrayfields['t.'.$key]['checked'])) {
				...
				} elseif ($key == 'taler_state') {
					// Display human-friendly Taler state label
					$state = (int) $object->taler_state;
					$map = [
						10 => 'Unpaid',
						20 => 'Claimed',
						30 => 'Paid',
