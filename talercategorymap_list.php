...
dol_include_once('/talerbarr/class/talercategorymap.class.php');
...
// Init
$object = new TalerCategoryMap($db);
...
if ($enablepermissioncheck) {
	$permissiontoread   = $user->hasRight('talerbarr', 'talercategorymap', 'read');
	$permissiontoadd    = $user->hasRight('talerbarr', 'talercategorymap', 'write');
	$permissiontodelete = $user->hasRight('talerbarr', 'talercategorymap', 'delete');
} else {
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'TalerCategoryMap';
	$objectlabel = 'TalerCategoryMap';
...
$title   = $langs->trans("TalerCategoryMaps");
$help_url = '';
$morejs = array();
$morecss = array();
...
foreach ($search as $key => $val) {
	if (array_key_exists($key, $object->fields)) {
		...
		if ((strpos($field_spec['type'], 'integer:') === 0) || (strpos($field_spec['type'], 'sellist:') === 0) || !empty($field_spec['arrayofkeyval'])) {
			if ($search[$key] == '-1' || ($search[$key] === '0' && (empty($field_spec['arrayofkeyval']) || !array_key_exists('0', $field_spec['arrayofkeyval'])))) $search[$key] = '';
			$mode_search = 2;
		}
		...
		if (empty($field_spec['searchmulti'])) {
			if (!is_array($search[$key]) && $search[$key] != '') $sql .= natural_search("t.".$db->escape($key), $search[$key], $mode_search);
		} else {
			if (is_array($search[$key]) && !empty($search[$key])) $sql .= natural_search("t.".$db->escape($key), implode(',', $search[$key]), $mode_search);
		}
	} else {
...
// Header
llxHeader('', $title, $help_url, '', 0, 0, array(), array(), '', 'mod-talerbarr page-list bodyforlist');
...
//$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/talerbarr/talercategorymap_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, $object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Massaction zone
$topicmail = "SendTalerCategoryMapRef";
$modelmail = "talercategorymap";
$objecttmp = new TalerCategoryMap($db);
$trackid = 'tcm'.$object->id;
...
if (!empty($moreforfilter)) print '<div class="liste_titre liste_titre_bydiv centpercent">'.$moreforfilter.'</div>';
...
if (!$conf->main_checkbox_left_column) print '<td class="liste_titre center maxwidthsearch">'.$form->showFilterButtons().'</td>';
print '</tr>'."\n";
