...
class TalerConfig extends CommonObject
{
	 ...
	 */
	public $element = 'talerconfig';

	/**
	 * @var string 		Name of table without prefix where object is stored. This is also the key used for extrafields management (so extrafields know the link to the parent table).
	 */
	public $table_element = 'talerbarr_talerconfig';

	/**
	 * @var string 		If permission must be checkec with hasRight('talerbarr', 'read') and not hasright('mymodyle', 'talerconfig', 'read'), you can uncomment this line
	 */
	//public $element_for_permission = 'talerbarr';

	/**
	 * @var string 		String with name of icon for talerconfig. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'talerconfig@talerbarr' if picto is file 'img/object_talerconfig.png'.
	 */
	...
	public $fields = array(
		...
		"talermerchanturl" => array("type" => "varchar(2048)", "label" => "TalerMerchantURL", "enabled" => "1", 'position' => 0, 'notnull' => 1, "visible" => "1", "comment" => "Url of the Taler Merchant"),
		...
		"taler_currency_alias" => array("type" => "varchar(16)", "label" => "TalerCurrencyAlias", "enabled" => "1", 'position' => 0, 'notnull' => 0, "visible" => "1", "help" => "TalerCurrencyAliasHelp", "comment" => "Currency code to treat as Dolibarr base currency when talking to Taler"),
		...
		"sync_on_paid" => array("type" => "integer", "label" => "SyncTiming", "enabled" => "1", 'position' => 0, 'notnull' => 0, "visible" => "1", "default" => "0", "arrayofkeyval" => array("0" => "SyncAtOrderCreated", "1" => "SyncAtOrderPaid"), "comment" => "When pulling from Taler, choose whether to sync at order creation or upon payment", "help" => "SyncTimingHelp"),
		"fk_bank_account" => ["type" => "integer:Account:compta/bank/class/account.class.php", "label"=>"BankAccount", "enabled"=>"1", "visible"=>"1", "notnull"=>1, "index"=>1, "comment"=>"Destination bank account for wired settlement"],
		"fk_default_customer" => ["type" => "integer:Societe:societe/class/societe.class.php", "label"=>"TalerDefaultCustomer", "enabled"=>"1", "visible"=>"1", "notnull"=>0, "index"=>1, "comment"=>"Fallback customer for autogenerated Taler orders"]
	);
	public $rowid;
	public $entity;
	...
	public $status;
	public $talermerchanturl;
	public $username;
	public $expiration;
	public $talertoken;
	public $taler_currency_alias;
	...
	private static ?string $currencyAlias = null;
	 ...
	 * @return void
	 */
	private function normalizeCurrencyAliasField(): void
	{
		$value = isset($this->taler_currency_alias) ? trim((string) $this->taler_currency_alias) : '';
		if ($value === '') {
			$this->taler_currency_alias = null;
			return;
		}
		$this->taler_currency_alias = strtoupper($value);
	}

	/**
	 * Update static currency alias cache from provided config.
	 *
	 * @param self|null $cfg Config instance (null resets to empty)
	 * @return void
	 */
	private static function primeCurrencyAlias(?self $cfg): void
	{
		if ($cfg instanceof self && !empty($cfg->taler_currency_alias)) {
			self::$currencyAlias = (string) $cfg->taler_currency_alias;
		} else {
			self::$currencyAlias = '';
		}
	}
	...
	//  */
	// public $table_element_line = 'talerbarr_talerconfigline';
	...
	//  */
	// public $fk_element = 'fk_talerconfig';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'TalerConfigline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'TalerConfig', 'fk_element'=>'fk_talerconfig'));
	...
	//  */
	// protected $childtablesoncascade = array('talerbarr_talerconfigdet');
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('talerbarr', 'talerconfig', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('talerbarr', 'talerconfig', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('talerbarr', 'talerconfig_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.$this->db->prefix()."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'talerconfig/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'talerconfig/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.$this->db->prefix()."ecm_files set filepath = 'talerconfig/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'talerconfig/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->talerbarr->dir_output.'/talerconfig/'.$oldref;
				$dirdest = $conf->talerbarr->dir_output.'/talerconfig/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->talerbarr->dir_output.'/talerconfig/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('talerbarr','write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('talerbarr','talerbarr_advance','validate'))))
		 {
		 $this->error='Permission denied';
		 return -1;
		 }*/

		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'TALERBARR_MYOBJECT_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'TALERBARR_MYOBJECT_CANCEL');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'TALERBARR_MYOBJECT_REOPEN');
	}
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowTalerConfig")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("TalerConfig").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/talerbarr/talerconfig_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowTalerConfig");
	...
	public function getLinesArray()
	{
		...
		$objectline = new TalerConfigLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, '(fk_talerconfig:=:'.((int) $this->id).')');
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('TALERBARR_MYOBJECT_ADDON')) {
			$conf->global->TALERBARR_MYOBJECT_ADDON = 'mod_talerconfig_standard';
		}

		if (getDolGlobalString('TALERBARR_MYOBJECT_ADDON')) {
			$mybool = false;

			$file = getDolGlobalString('TALERBARR_MYOBJECT_ADDON').".php";
			$classname = getDolGlobalString('TALERBARR_MYOBJECT_ADDON');

			// Include file with class
			$dirmodels = array_merge(array('/'), (array) $conf->modules_parts['models']);
			foreach ($dirmodels as $reldir) {
				$dir = dol_buildpath($reldir."core/modules/talerbarr/");
			...
			if (class_exists($classname)) {
				$obj = new $classname();
				'@phan-var-force ModeleNumRefTalerConfig $obj';
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		$modelpath = "core/modules/talerbarr/doc/";
	...
	{
		$tmp = new self($db);
		$table = MAIN_DB_PREFIX.$tmp->table_element;

		$entityFilter = 'entity IN ('.getEntity($tmp->element, true).')';

		$sql = "SELECT rowid FROM ".$table;
		if ($entityFilter !== '') {
			$sql .= ' WHERE '.$entityFilter;
		}
		...
		if ($db->num_rows($resql) == 0) {
			$error = 'No config was find, create a new.';
		...
		$obj->verification_ok = false;
		$obj->verification_error = null;

		if (!$obj->verifyConfig($error)) {
			// Mark as invalid but return the object so UI can prefill
			$obj->verification_ok = false;
			$obj->verification_error = ($error ?: 'Config is invalid');
			self::primeCurrencyAlias($obj);
			return $obj;
		}

		// Valid
		$obj->verification_ok = true;
		$obj->verification_error = null;
		self::primeCurrencyAlias($obj);
		return $obj;
	}
	 ...
	 *
	 * @param string|null $error Filled on failure
	 * @return bool
	 */
	public function verifyConfig(?string &$error = null): bool
	{
		$error = null;

		// read fields defensively
		$baseUrl  = isset($this->talermerchanturl) ? trim((string) $this->talermerchanturl) : '';
		$username = isset($this->username)        ? trim((string) $this->username)        : '';
		$token    = !empty($this->talertoken) ? $this->talertoken : '';

		// 1) Normalize + validate base URL
		$baseUrl = self::normalizeMerchantUrl($baseUrl);
		if (!$baseUrl) {
			$error = 'Empty or invalid Taler Merchant URL';
			return false;
		}
		if (!filter_var($baseUrl, FILTER_VALIDATE_URL)) {
			$error = 'Taler Merchant URL is not a valid URL: '.$baseUrl;
			return false;
		}
	 ...
	 * @return string
	 */
	public static function getDolibarrCurrency(): string
	{
		global $conf;
		$code = isset($conf->currency) && $conf->currency !== '' ? (string) $conf->currency : 'EUR';
	 ...
	 *
	 * @return string Uppercase alias (empty string when none configured)
	 */
	public static function getCurrencyAlias(): string
	{
		return self::$currencyAlias ?? '';
	}

	/**
	 * Map a Taler-reported currency into a Dolibarr currency code.
	 *
	 * @param string      $currency Currency code received from Taler
	 * @return string               Uppercase currency suitable for Dolibarr
	 */
	public static function mapCurrencyFromTaler(string $currency): string
	{
		$normalized = strtoupper(trim($currency));
		if ($normalized === '') {
			return '';
		}
		$alias = self::getCurrencyAlias();
		if ($alias !== '' && $normalized === $alias) {
			return self::getDolibarrCurrency();
		}
		return $normalized;
	}
	 ...
	 *
	 * @param string      $currency Dolibarr currency code
	 * @return string               Uppercase currency to send to Taler
	 */
	public static function mapCurrencyToTaler(string $currency): string
	{
		$normalized = strtoupper(trim($currency));
		if ($normalized === '') {
			return '';
		...
		$alias = self::getCurrencyAlias();
		if ($alias !== '' && $normalized === self::getDolibarrCurrency()) {
			return $alias;
		}
		return $normalized;
	}
	...
	public static function buildInstanceBase(string $baseUrl, string $username): string
	{
		...
		return $base.'/instances/'.rawurlencode($username).'/private/';
	}

	/**
	 * Normalize a merchant URL:
	 *  - trim
	...
	private static function isUrlOk(string $url, array $headers, int &$httpcode, string &$err): bool
	{
		...
		// Perform GET request with optional headers
		$res = getURLContent(
			$url,
			'GET',          // method
			'',             // no params for GET
		...
		$httpcode = isset($res['http_code']) ? (int) $res['http_code'] : 0;

		if (!empty($res['curl_error_no'])) {
			$err = 'cURL#'.$res['curl_error_no'].($res['curl_error_msg'] ? ' '.$res['curl_error_msg'] : '');
		}
 ...
 */
class TalerConfigLine extends CommonObjectLine
{
	// To complete with content of an object TalerConfigLine
	// We should have a field rowid, fk_talerconfig and position

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $parent_element = '';		// Example: '' or 'talerconfig'

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $fk_parent_attribute = '';	// Example: '' or 'fk_talerconfig'
